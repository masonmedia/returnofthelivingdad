    ...
    <div class="container-fluid p-0 border-top border-warning">
        <div class="row text-warning p-4">
            ...
            <div class="col-lg-3 col-md-6 d-flex flex-column align-items-start justify-content-start p-4">
                <h3 class="ls-base fw-900 text-uppercase mt-0">{{ postWithId[0].title }}</h3>
                <p class="p fw-bold my-2" v-html="shorten(postWithId[0].body, 150) + '...'"></p>
                <router-link :to="'/' + postWithId[0].id + '/' + postWithId[0].slug">
                  <button class="btn btn-sm btn-outline-warning px-4">More</button>
                </router-link>
            </div>
            <div class="col-lg-3 col-md-6 p-4">
                <img loading="lazy" :src="postWithId[0].imageUrl" class="fade-in rounded-3 w-100" style="min-height: 200px" :alt="postWithId[0].title">
            </div>
